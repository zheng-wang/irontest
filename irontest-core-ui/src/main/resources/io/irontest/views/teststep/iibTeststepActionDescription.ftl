...
<#if stepRun.teststep.endpoint.type == 'MQ'>through channel "${ (stepRun.teststep.endpoint.otherProperties.svrConnChannelName??)?then(stepRun.teststep.endpoint.otherProperties.svrConnChannelName, 'null') }".</#if>